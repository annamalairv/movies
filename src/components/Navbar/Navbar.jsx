      ...
      {!search ? (
        <div className="flex flex-1 items-center font-medium">
          ...
          <div className="flex-1 flex flow-row justify-end items-center">
            ...
            <div
              className="md:block hidden px-5 text-2xl  cursor-pointer"
            ...
            <ul
              className={`
        md:hidden bg-[#2A2A2A] fixed w-full top-0 overflow-y-auto bottom-0 py-24 pl-4 z-50
        ...
        >
          <div className="text-sm flex-1 flex flex-row items-center">
            ...
            <input
              autoFocus={true}
              type={isQueryTypeMovieName ? "text" : "number"}
              value={query}
              className="block w-full px-2 py-2 text-purple-700 bg-white border rounded-full focus:border-[#3e4555] focus:ring-[#3e4555] focus:outline-none focus:ring focus:ring-opacity-40"
              placeholder="Search by"
              onChange={setSearchs}
            />
            {query && query.length != 4 && !isQueryTypeMovieName ? (
              <div className="text-xs text-red-400 px-3 whitespace-nowrap">
                Enter a valid year
              </div>
            ) : null}
          </div>
